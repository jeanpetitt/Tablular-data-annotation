...
def main():
    ...
    t5.loginHub(token=sys_args.hf_token)
    model = t5.load_model(
        model_id=sys_args.model_id,
        device_map={"": 0},
        use_peft=True
    )
    tokenizer = t5.load_tokenizer(model_id=sys_args.model_id)
    formatting_func = t5.format_instruction
    trainer = t5.trainer(
        model=model,
